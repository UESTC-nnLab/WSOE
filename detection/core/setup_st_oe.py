...
from tools_st_oe.plain_generalized_rcnn_logistic_ws_st_oe import GeneralizedRCNNLogisticWSSTOE
from tools_st_oe.roihead_ws_st_oe import ROIHeadsLogisticWSOEBOXST
from tools_st_oe.rpn_oe import RPN_OE
...
def setup_arg_parser():
    ...
    arg_parser.add_argument(
        ...
        type=str,
        default="temp",
        help="path to OE dataset directory")    
...
def add_oe(cfg):
    _C = cfg
    _C.AUXI_DATASETS = CN()
    _C.AUXI_DATASETS.TRAIN= ('ImageNet1k_Val_OE',)
    _C.AUXI_DATASETS.ID_OD_RATIO = 0.5
    _C.AUXI_DATASETS.START_FINETUNE_ITER = 48000
    _C.AUXI_DATASETS.NUM_CLASSES = 20
    _C.AUXI_DATASETS.BATCH_SIZE_PER_IMAGE = 128
    _C.AUXI_DATASETS.POSITIVE_FRACTION = 1
    _C.AUXI_DATASETS.SUBSET = 15000
    # _C.AUXI_DATASETS.IS_SUBSET = False
    _C.AUXI_DATASETS.OE_IOU_THRESHOLDS = [0.3,0.7]
    _C.AUXI_DATASETS.OE_IOU_LABELS=[0,-1, 1]
    _C.SOLVER.LOSS_WEIGHT = 0.2

def add_st_oe(cfg):
    _C = cfg
    _C.SELF_TRAINING = CN()
    _C.SELF_TRAINING.NUM_ROUNDS = 5
    _C.SELF_TRAINING.ITERATIONS_PER_ROUND= 12000
    _C.SELF_TRAINING.KC_POLICY = 'None'
    _C.SELF_TRAINING.KC_VALUE = 'energy_score'
    _C.SELF_TRAINING.INIT_OE_PORTION = 1.0
    _C.SELF_TRAINING.MIN_OE_PORTION = 1.0
    _C.SELF_TRAINING.OE_PORTION_STEP = 0.05
    _C.SELF_TRAINING.RAW_RESULTS_PATH = 'raw_results_file'
    _C.SELF_TRAINING.FILTERED_RESULTS_PATH = 'filtered_results_file'
    _C.SELF_TRAINING.PSEUDO_LABEL_SAVED_PATH = "ImageNet1k_Val_OE_Pseudo_Label"
    _C.SELF_TRAINING.LABEL_PATH = "true_label_file"
    _C.SELF_TRAINING.START_SELF_SPACED = 24000
    _C.SELF_TRAINING.PSEUDO_BOX_STRATEGY = "pred_box"
...
def setup_config(args, random_seed=None, is_testing=False, ood=False):
    ...
    cfg['OE_DATASET_PATH'] = os.path.expanduser(args.oe_dataset_dir)
    ...
    oe_dataset_dir = os.path.expanduser(args.oe_dataset_dir)
