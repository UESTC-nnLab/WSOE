...
from tools_st_oe.box_regression_st import Box2BoxTransform, _dense_box_regression_loss
...
def fast_rcnn_inference(
    ...
    Args:
        inter_feats(list[Tensor]):A list of Tensors of inter feartures
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            inter_feats_per_image, boxes_per_image, scores_per_image, image_shape, score_thresh, nms_thresh, topk_per_image
        )
        for inter_feats_per_image, scores_per_image, boxes_per_image, image_shape in zip(inter_feats, scores, boxes, image_shapes)
    ]
...
def fast_rcnn_inference_single_image(
    ...
    # energy score
    energy_scores = log_sum_exp(inter_feats, 1)[filter_inds[:,0]]
    # energy_scores = log_sum_exp(_scores, 1)[filter_inds[:, 0]]
    ...
    energy_scores = energy_scores[keep]

    result = Instances(image_shape)
    result.pred_boxes = Boxes(boxes)
    result.box_sizes = result.pred_boxes.area()
    # result.max_box_id = result.box_sizes.argmax() if result.box_sizes.size()[0] >0 else torch.full([1,],-1,dtype=torch.long)
    result.scores = scores
    result.pred_classes = filter_inds[:, 1] +1
    result.energy_scores = energy_scores
    # result.cls_prop = _scores[keep]
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        """
        # inter_feat , _ = predictions
        boxes = self.predict_boxes(predictions, proposals)
        scores = self.predict_probs(predictions, proposals)
        inter_feats = self.process_inter_feat(predictions, proposals)
    ...
    def process_inter_feat(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals:List[Instances]):
        inter_feats, _ = predictions
        num_inst_per_image = [len(p) for p in proposals]
        return inter_feats.split(num_inst_per_image,dim =0)
