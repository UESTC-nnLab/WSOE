...
from tools_st_oe.roihead_ws_st_oe import build_roi_heads
from detectron2.modeling.meta_arch.build import META_ARCH_REGISTRY

__all__ = ["GeneralizedRCNNLogisticWSSTOE"]


@META_ARCH_REGISTRY.register()
class GeneralizedRCNNLogisticWSSTOE(nn.Module):
    """
    ...
    def forward(self, batched_inputs: Tuple[Dict[str, torch.Tensor]], iteration = 0):
        ...
        if self.proposal_generator is not None:
            proposals, proposal_losses, oe_mask, id_mask = self.proposal_generator(images, features, gt_instances)

        else:
            assert "proposals" in batched_inputs[0]
            proposals = [x["proposals"].to(self.device) for x in batched_inputs]
        # breakpoint()
        _, detector_losses = self.roi_heads(images, features, proposals, iteration, gt_instances, oe_mask, id_mask)
